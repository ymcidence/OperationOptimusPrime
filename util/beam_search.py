...
def _log_prob_from_logits(logits, reduce_axis=-1):
...
def fast_tpu_gather(params, indices, name=None):
    ...
    with tf.name_scope(name):
        ...
        def _gather(params, indices):
            ...
            shape = _shape_list(params)
            indices_shape = _shape_list(indices)
        ...
        if dtype.is_integer:
            gather_result = tf.gather(params, indices, batch_dims=-1)
        else:
